...
public Action StartTouchTrigger(int caller, int activator)
{
	...
	else if (iZoneType == ZONETYPE_END)
	{
		if (g_iClientInZone[client][2] == iZoneGroup) // Cant end bonus timer in this zone && in the having the same timer on
		{
			...
			{
				if (g_hMustPassCheckpoints.BoolValue && g_iTotalCheckpoints > 0 && iZoneGroup == 0)
				{
	...
	else if (iZoneType == ZONETYPE_CHECKPOINT)
	{
		if (iZoneTypeId != lastCheckpoint[g_iClientInZone[client][2]][client] && g_iClientInZone[client][2] == iZoneGroup)
		{
			...
			// Checkpoint enforcer
			if (g_hMustPassCheckpoints.BoolValue && g_iTotalCheckpoints > 0)
			{
				g_iCheckpointsPassed[client]++;
...
public Action EndTouchTrigger(int caller, int activator)
{
	...
	if (iZoneGroup != g_iClientInZone[activator][2] || iZoneType == ZONETYPE_TELETOSTART || iZoneType == ZONETYPE_CHECKER || iZoneType != g_iClientInZone[activator][0])
	{
...
public int Handle_ZoneMenu(Handle tMenu, MenuAction action, int client, int item)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			switch (item)
			{
				case 0: SelectZoneGroup(client); // Create a zone
				
				case 1: EditZoneGroup(client); // Edit Zones
				...
				case 3: ZoneSettings(client); // Edit Zone Settings
...
public void ZoneSettings(int client)
{
	...
	switch (g_hZoneDisplayType.IntValue)
	{
	...
	switch (g_hZonesToDisplay.IntValue)
	{
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			switch (item)
			{
				case 0:
				{
					if (g_hZoneDisplayType.IntValue < 2)
						SetConVarInt(g_hZoneDisplayType, (g_hZoneDisplayType.IntValue + 1));
					else
				...
				case 1:
				{
					if (g_hZonesToDisplay.IntValue < 3)
						SetConVarInt(g_hZonesToDisplay, (g_hZonesToDisplay.IntValue + 1));
					else
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			switch (item)
			{
				...
				case 6: ScaleMenu(client, 0); // Scaling

				case 7: ChangeZonesHook(client);
				...
				case 10: PrespeedMenu(client); // prespeed
			}
		}
